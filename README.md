...
content/posts/    the blog: <slug>.md — the pre-commit hook stamps the
                  YYYY-MM-DD- prefix at commit time (title in the H1)
...
.githooks/        pre-commit (stamps post dates) + post-commit (rebuilds dist/)
```
...
git config core.hooksPath .githooks   # one-time, enables the hooks
```

- add a page: `content/foo.md` → commit → `/foo` (and raw `/foo.md`)
- add a post: `content/posts/<slug>.md` (any name) → commit — the
  pre-commit hook stamps the date, so the committed file is
  `YYYY-MM-DD-<slug>.md` → the page `/posts/YYYY-MM-DD-<slug>`, listed at
